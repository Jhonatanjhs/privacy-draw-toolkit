(function() {
    ...
    let currentTool = 'pen', lastTool = 'pen', redoStack = [], isDown = false, activeText = null;
    ...
    toolbar.innerHTML = `
        ...
        <div style="display:flex; flex-direction:column; gap:3px; margin-top:5px;">
            <button class="draw-btn action-btn" id="copyBtn" title="Copy page + drawing to clipboard (Ctrl+C)">📋 Copy to Clipboard</button>
            <div style="display:flex; gap:3px;">
                <button class="draw-btn action-btn" id="saveBtn" style="flex:1; background:#28a745; border-color:#28a745;" title="Save page + drawing as PNG (Ctrl+S)">💾</button>
                <button class="draw-btn action-btn" id="clearBtn" style="flex:1; background:#4fc3f7; border-color:#4fc3f7; color:#222;" title="Clear canvas (Ctrl+X)">🧹</button>
                <button class="draw-btn action-btn" id="exitBtn" style="flex:1; background:#dc3545; border-color:#dc3545;" title="Exit toolkit">✖</button>
            </div>
        </div>
    `;
    ...
    const setTool = (tool) => {
        // Clicking the already-active tool → go to browsing mode
        if (tool === currentTool) {
            enterBrowsingMode();
            return;
        }
    ...
    chrome.storage.sync.get(['brushColor', 'highlighterColor', 'textColor', 'penSize', 'highlighterSize', 'eraserSize', 'textSize'], (prefs) => {
        if (prefs.brushColor) { brushColor = prefs.brushColor; syncColor(brushColor); }
        if (prefs.highlighterColor) { highlighterColor = prefs.highlighterColor; document.getElementById('highlighterColorPicker').value = highlighterColor; }
        if (prefs.textColor) { textColor = prefs.textColor; document.getElementById('textColorPicker').value = textColor; }
        if (prefs.penSize) penSize = prefs.penSize;
        if (prefs.highlighterSize) highlighterSize = prefs.highlighterSize;
        if (prefs.eraserSize) eraserSize = prefs.eraserSize;
